...
def parse_arguments():
    ...
    parser.add_argument(
        "--sweep_lambda_index", type=int,
        help="Index of the lambda to sweep.")
...
from classifim.xxz1d_w import WPipeline

def init_config(args):
    sweep_lambda_index = args.sweep_lambda_index
    assert sweep_lambda_index in (0, 1), (
        f"{sweep_lambda_index=}; {type(sweep_lambda_index)=}")
    ...
    model_sweep_prefix = f"{model_name}_{suffix}_{config['sweep_lambda_index']}"
    config.update({
        "model_filename": os.path.join(
            models_dir, f"{model_sweep_prefix}_" + "{lambda_fixed_i}.pth"),
        "log_filename": os.path.join(
            models_dir, f"{model_sweep_prefix}.log.json"),
        "w_filename": os.path.join(
            models_dir, f"{model_sweep_prefix}.w.npz")
    })
    return config
...
def run_pipeline(config):
    ...
    pipeline = WPipeline(config=config)
    cur_log["train"] = []
    t2 = t1 = time.time()
    dt_train = 0
    dt_eval = 0
    for lambda_fixed_i in range(64):
    ...
    with open(config["log_filename"], "w") as f:
        json.dump(cur_log, f)
    print(f"{datetime.datetime.now()}: Done; log @ '{config['log_filename']}'")
